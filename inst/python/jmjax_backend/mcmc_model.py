...
# Absorbable-direction machinery and the rotation built on it (used by the
# recommended control$rotate_absorbable), and the legacy sweep (not
# recommended; kept runnable for reproducibility). Re-exported names keep
# existing callers such as dev/verify_absorbable_basis.py working.
from .absorbable import (_absorbable_basis, _absorbable_basis_exact,  # noqa: F401
                         _structural_extension_residual, _union_basis,
                         _householder_reflectors, _apply_reflectors)
from . import sweep as _sweep
from .sweep import _absorbable_generator_matrix, _orthogonal_complement  # noqa: F401
            ...
            if random_effects_method == "wishart_gibbs":
                ...
                if random_effects_corr:
                    ...
                    if gen_reflectors is not None and q >= 2:
                        ...
                        # LEGACY (control$orthogonalize_b0_rotate): see sweep.py.
                        b_std = _sweep.sample_b_std_b0_rotated(
                            b_orth_bases, b0_gen_perp, N_sub, q)
                    else:
                        with numpyro.plate("subjects", N_sub):
                    ...
                    if b_orth_bases is None or not any(
                        ...
                        # LEGACY (control$orthogonalize_b0/_b): see sweep.py.
                        b = _sweep.apply_sweep(b_raw, b_orth_bases, q)
                else:
...
def fit_nuts(X_long, y_long, n_obs, X_time_surv, X_time_quad,
    ...
    # ------------------------------------------------------------------
    # LEGACY: the location sweep (control$orthogonalize_b0/_b and its
    # rotations) lives in sweep.py. It is not part of jmjax's recommended
    # path - see that module's docstring - and sweep.setup() returns all
    # None/False unless one of its options is requested.
    # ------------------------------------------------------------------
    _sw = _sweep.setup(control, q, random_effects_corr, random_effects_method,
                       X_long, Z_long, n_obs, X_time_surv, Z_time_surv,
                       X_time_quad, Z_time_quad, N_sub)
    b_orth_bases = _sw["b_orth_bases"]
    _correction_bases = _sw["correction_bases"]
    orthogonalize_report = _sw["report"]
    _b0_gen_perp = _sw["b0_gen_perp"]
    _gen_reflectors = _sw["gen_reflectors"]
    _orth_all, _orth_int = _sw["orth_all"], _sw["orth_int"]
    _orth_int_rotate, _rot_all = _sw["orth_int_rotate"], _sw["rot_all"]

    # control$rotate_absorbable (+ control$dense_mass_generator_beta): jmjax's
    # approach to the location degeneracy. Every column of b_std is rotated
    # by one fixed Householder matrix whose first k columns span the
    # absorbable directions (absorbable.py), so the degeneracy lives in the
    # explicit [k, q] site b_gen_U; the dense block over (beta, b_gen_U)
    # then handles the beta <-> b_gen_U ridge. b is NOT projected, nothing
    # needs correcting, and the reported beta is the ordinary one. Measured:
    ...
    # LEGACY (control$dense_mass_b0_generator): see sweep.py.
    _blocks.extend(_sweep.dense_blocks(control, _b0_gen_perp))
        ...
        if "b_std" in _vals and (_gen_reflectors is not None
            ...
            try:
                ...
                if _gen_reflectors is not None:
                    ...
                    _vals.update(_sweep.warm_start_b0_rotated(
                        _bs, b_orth_bases, _b0_gen_perp))
    ...
    # LEGACY: "beta_corrected" for the sweep options (sweep.py). A no-op
    # unless orthogonalize_b0/_b swept at least one direction.
    _sweep.add_beta_corrected(samples, _correction_bases, b_orth_bases)
