...
All code is in the code archive, which contains the \texttt{jmjax} package
source. The development scripts below reproduce each result; paths are
...
Table~\mref{tab:real} & \texttt{dev/study\_realdata\_rotate.R} \\
Table~\mref{tab:jmb}; JMbayes2 in Figure~\mref{fig:alpha} & \texttt{dev/study\_common\_ess.R} (\texttt{dev/common\_ess.csv}) \\
Section~\ref{sec:realq1} & \texttt{dev/pilot\_q1\_intercept.R} \\
