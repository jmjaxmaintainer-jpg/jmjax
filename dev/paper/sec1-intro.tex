...
\citep{neal2003,betancourt2015} and is the reason these samplers work well
on hierarchical models at all.
...
value unchanged. The likelihood identifies the sum; only the prior on the
random effects, at scale $\sigma_0/\sqrt{N}$ for $N$ subjects, pins the
split. The same holds for every subject-constant covariate, whose
...
ridge along them, and the sampler mixes slowly on the intercept, the
...
This paper does something different. It keeps the absorbable directions,
...
then whitens the ridge. The rotation acts on the subject index and the
...
hazard ratio per unit of the current marker value. On it, NUTS was 3 to 8
times more efficient than the Metropolis-within-Gibbs sampler of JMbayes2
\citep{jmbayes2} with no reparameterization at all. We show why: for an
ideal two-block sampler that draws $\alpha$ exactly given the rest, the
lag-1 autocorrelation equals the share of $\alpha$'s posterior variance
the rest explains \citep{liu1994}, and in joint models that share is large
for a structural reason, the trade-off between $\alpha$ and the level of
...
is weakest; a stress grid aimed at those weak spots kept the gains at
5--30 times.
