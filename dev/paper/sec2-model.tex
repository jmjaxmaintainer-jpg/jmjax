...
Gaussian in the linear model (Proposition~\ref{prop:ridge} gives its
covariance in closed form), so the curvature along the ridge is constant.
The problem is global and linear: the ridge is narrow in a direction a
diagonal metric cannot represent, which is exactly what a fixed Euclidean
metric can correct once the direction is exposed.
