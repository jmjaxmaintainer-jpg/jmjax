...
of the model rather than of the observed follow-up. The definition concerns
...
\begin{enumerate}
\item[(a)] $\A_c^{\mathrm{long}} = \U_c^{-1}(\range \T) =
\ker\{(I - P_{\T})\,\U_c\}$. It depends on the fixed-effects design only
...
coincidence at the observed times. More generally it holds whenever the
functions involved lie in a space determined by their values at a
subject's observed times, such as polynomials of degree below $n_i$. It
can fail for subjects with few observations relative to the flexibility of
the time basis. It is also cheap to check: the design rows at the event
