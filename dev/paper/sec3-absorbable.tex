...
orthogonal matrix. What decides whether it does anything useful is the
basis it is built from. The basis should contain every direction the
fixed effects can absorb, so that none is left spread across the $N$
subject coordinates, and only such directions, so that the dense block is
not spent on directions the data pin down. This section characterizes them.
...
(\ref{eq:T}) on all of $[0, \tau]$ is slightly more than invariance of
the likelihood needs; it is what makes the set of directions a property
of the model rather than of the observed follow-up. $\A_c$ is therefore a
structural set of invariant directions, sufficient for likelihood
invariance; it need not be the largest set of shifts that leaves a
particular observed likelihood numerically unchanged. The definition concerns
the likelihood only: the prior on the random effects is not invariant under
these shifts, and that asymmetry is what identifies the split at all.
...
column $c$ that enters the likelihood, so we may take $S_c$ to be every
subject. Then one singular value decomposition of the $(\sum_i n_i) \times
...
The proof is in Appendix~\ref{app:proofs}. Condition~\ref{cond:S} holds for subject-constant, linear-time and covariate-by-time columns: a subject-constant column
($x_i(t)^{\top}e_j \equiv \text{const}$, absorbed by the intercept column
$z_i(t) \equiv 1$), the time column ($x_i(t)^{\top}e_j = t$, absorbed by a
random slope $z_i(t) = t$) and a covariate-by-time interaction
($c_i t$). In each the identity holds as a function of $t$, not by
coincidence at the observed times. More generally it holds when, for each
subject, the functions $t \mapsto x_i(t)^{\top} d$ and $t \mapsto z_i(t)$
are known to lie in a finite-dimensional function space determined by its
values at the subject's distinct observation times, for example
polynomials of degree below $n_i$ (Appendix~\ref{app:proofs}). For spline
bases this depends on the knots and on where the observations fall, and
it can fail for subjects with few observations relative to the
flexibility of the basis. It is also cheap to check: the design rows at the event
times and quadrature nodes are needed for the likelihood anyway, and
\texttt{jmjax} evaluates the identity there at every fit and warns if the
worst relative residual exceeds $10^{-6}$. Proposition~\ref{prop:joint}
also settles how to treat a subject with no observation at which the
column is nonzero, such as a baseline-only subject for the slope column:
its entry is forced by the survival submodel, not free, and setting it to
zero would produce a direction the likelihood does identify.
...
exactly what an orthogonal-polynomial or natural-spline mean in time
produces; the monomial spelling \texttt{time + I(time\^{}2)} is caught,
because there the time column appears on its own. So with a spline mean
and a random slope, the column-by-column search silently leaves the slope
degeneracy in place, while still reporting a basis for the intercept
column. By Proposition~\ref{prop:char}(a) the absorbable set depends only
on the column space, and the null-space construction of
Proposition~\ref{prop:char}(b) is invariant to how that space is spanned.
It is what \texttt{jmjax} now uses; the column-wise search is retained
only to report which covariates expose each direction.
