# jmjax development log, 0.2.0 to 0.3.0

This is the full, chronological record of the changes that became jmjax
0.3.0, newest first, as it was kept in `NEWS.md` during development: the
measurements, the failed attempts and the reasoning behind each default.
`NEWS.md` itself now carries the user-facing summary; nothing here was
edited when it moved, so some entries describe states that later entries
supersede (for example, `random_effects_method = "wishart_gibbs"` was once
recommended and later withdrawn).
